...
def prefetch_test(opt):
  ...
  for ind, (img_id, pre_processed_images,img_info) in enumerate(data_loader):
    ...
    if opt.tracking and ('is_first_frame' in pre_processed_images):
      ...
      if opt.dataset == 'nuscenes':
        save_video_name = os.path.join(opt.dataset + '_videos/', 'MOT'+str(int(pre_processed_images['video_id']))+'_'+str(int(img_info['sensor_id']))+'.avi')
      elif opt.dataset == 'kitti_tracking':
        save_video_name = os.path.join(opt.dataset + '_videos/', 'KITTI_'+str(int(pre_processed_images['video_id']))+'.avi')
      else:
        save_video_name = os.path.join(opt.dataset + '_videos/', 'MOT'+str(int(pre_processed_images['video_id']))+'.avi')
      ...
      for video in dataset.coco.dataset['videos']:
        ...
        if pre_processed_images['video_id'] == video_id and not opt.dataset == 'nuscenes' :
    ...
    for t in online_targets:
      ...
      if tlwh[2] * tlwh[3] > min_box_area:
        ...
        if opt.dataset == 'nuscenes':
          ...
          translation, size, rotation = ddd_box_submission[:3],ddd_box_submission[3:6],ddd_box_submission[6:]
    ...
    final_results.append((pre_processed_images['frame_id'].cpu().item() , online_tlwhs, online_ids))
    if show_image :
      img0 = pre_processed_images['image'][0].numpy()
      ...
      if opt.dataset == 'nuscenes':
          online_im = plot_tracking_ddd(img0, online_tlwhs, online_ddd_boxes, online_ids, frame_id=pre_processed_images['frame_id'],calib=img_info['calib'])
      else:
          online_im = plot_tracking(img0, online_tlwhs, online_ids, frame_id=pre_processed_images['frame_id'])
      vw.write(online_im)
  ...
  if opt.dataset == 'nuscenes':
      ...
      json.dump(ret, 
                    open(results_dir+'/results.json', 'w'))
