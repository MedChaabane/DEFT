...
class AFE_module(nn.Module):
    ...
    def __init__(self,opt):
        ...
        if opt.dataset == 'nuscenes':
            SELECTOR_OUTPUT_CHANNEL = [48,48,64,64,64,64,64,64,64,64,32,32,32]
       
        else:
            SELECTOR_OUTPUT_CHANNEL = [32,32,32,32,32,32,32,32,32,32,32,32,32]
        ...
        self.selector = nn.ModuleList(selector(self.selector_input_channel, self.selector_output_channel))
            ...
            for source_index in range(len(sources)):
                ...
                label_res.append(
                    ...
                    F.grid_sample(sources[source_index],  # [B, C, H, W]
                                  ...
                                  , padding_mode="border").squeeze(2).squeeze(2)
                )
...
def selector(selector_input_channel, selector_output_channel):
    ...
    for k, v in enumerate(selector_input_channel):
         selector_layers += [nn.Conv2d(selector_input_channel[k],
...
def show_batch_circle_image(img_pre, img_next, boxes_pre, boxes_next, valid_pre, valid_next, indexes,opt):
    ...
    for b in boxes1:
        ...
        img1 = cv2.circle(img1, tup, 20, [0, 0, 255],
                          thickness=3)
    ...
    for b in boxes2:
        ...
        b[1]=(b[1]+1)/ 2.0 *height
        tup = tuple(b.astype(int))
        img2 = cv2.circle(img2, tup, 20, [0, 0, 255],
                          thickness=3)
