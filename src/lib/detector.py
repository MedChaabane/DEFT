...
#     self.pause = not opt.no_pause
  ...
  # public detection mode in MOT challenge
    if self.opt.public_det:
        results = pre_processed_images['meta']['cur_dets'] if self.opt.public_det else None
    ...
    if self.dataset == 'nuscenes':
        ...
        for det in results:
            ...
            if class_name == 'pedestrian' and det['score']  <0.35:
                continue
            results_by_class[class_name].append(det['bbox'].tolist() + [det['score']])
            size = [float(det['dim'][1]), float(det['dim'][2]), \
            float(det['dim'][0])]
            rot_cam = Quaternion(
            axis=[0, 1, 0], angle=det['rot_y'])
            translation_submission1 = np.dot(trans_matrix, np.array(
            ...
            ddd_org_boxes_by_class[class_name].append([float(det['dim'][0]), float(det['dim'][1]), \
            float(det['dim'][2])]+trans+[det['rot_y']])
            ...
            ddd_box_submission1[class_name].append([float(translation_submission1[0]), float(translation_submission1[1]), float(translation_submission1[2])].copy()+ size.copy()+rotation.copy() )
            ...
            ddd_boxes_by_class[class_name].append([size[2], size[0], size[1], 
        ...
        for class_name in NUSCENES_TRACKING_NAMES:
            if len(results_by_class[class_name])>0 and NMS:
                boxess= torch.from_numpy(np.array(results_by_class[class_name])[:,:4])
                scoress= torch.from_numpy(np.array(results_by_class[class_name])[:,-1])
                ...
                results_by_class[class_name]=np.array(results_by_class[class_name])[keep]
                
                ddd_boxes_by_class[class_name]=np.array(ddd_boxes_by_class[class_name])[keep]
                depths_by_class[class_name]=np.array(depths_by_class[class_name])[keep]
                ddd_org_boxes_by_class[class_name]=np.array(ddd_org_boxes_by_class[class_name])[keep]
                ddd_box_submission1[class_name]=np.array(ddd_box_submission1[class_name])[keep]

                


            online_targets+= self.tracker[class_name].update(results_by_class[class_name],FeatureMaps,ddd_boxes=ddd_boxes_by_class[class_name],depths_by_class=depths_by_class[class_name],ddd_org_boxes=ddd_org_boxes_by_class[class_name],submission = ddd_box_submission1[class_name],classe = class_name)
  ...
  def reset_tracking(self,opt):
    if self.dataset == 'nuscenes':
        self.tracker={}
        for class_name in NUSCENES_TRACKING_NAMES:
            self.tracker[class_name] = Tracker(opt,self.model,h=self.img_height, w=self.img_width)
    else:
