    ...
    if index not in self.possible_tracks:
        ...
        while not valid:
            ...
            if index not in self.valid_indexes:
                ...
                img_ids = [img_info['frame_id'] \
                      for img_info in img_infos \
                      if abs(img_info['frame_id'] - frame_id) <= self.MAX_dis and \
        ...
        for ind in range(index-self.MAX_dis,index+1):
            ...
            if ind>index-2 or randint(0, 10)<9:
                if track_id in track_ids :
                    ...
                    h, w, l, c_x, c_y, c_z, rot_y= bbox[0], bbox[1], bbox[2], bbox[3], bbox[4], bbox[5],bbox[6]
                    if last_t>-1:
                        delta_h = h-last_h
                        delta_w = w-last_w
                        delta_l=l-last_l
                    ...
                    input_traj.append([ c_x, c_y, c_z,delta_cx,delta_cy,delta_cz, h, w,l, delta_h,delta_w,delta_l, v_x,v_y,v_z,rot_y,delta_rot,v_rot].copy()) 
        ...
        for ind in range(index-self.MAX_dis,index+1):
            ...
            if ind>index-2 or randint(0, 10)<9:
                ...
                if track_id in track_ids :
                    ...
                    input_traj.append([ c_x,c_y,delta_cx,delta_cy, h, w,h_w_ratio,delta_h,delta_w,v_x,v_y].copy())   
                    last_t=t
                    last_h=h
                    last_w=w
                    last_cx=c_x
  ...
  def _get_bboxes(self, anns,image_info=None):
    ...
    if self.dataset == 'nuscenes':
        for ann in anns:
            ...
            size = [float(wlh[1]), float(wlh[2]), \
            float(wlh[0])]
            
            rot_cam = Quaternion(
            axis=[0, 1, 0], angle=rotation)
            loc = np.array(
            [center_location[0],center_location[1], center_location[2]], np.float32).copy()
            
            translation = np.dot(trans_matrix, np.array(
            [center_location[0], center_location[1] - size[2]/2, center_location[2], 1], 
            np.float32)).copy()
            ...
            bboxes.append([size[2], size[0], size[1], 
  ...
  def _load_pre_data_AFE(self, video_id, frame_id, sensor_id=1):
    ...
    if 'train' in self.split:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          for img_info in img_infos \
          if abs(img_info['frame_id'] - frame_id) < self.opt.max_frame_dist_AFE and \
          (not ('sensor_id' in img_info) or img_info['sensor_id'] == sensor_id)]
    else:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          for img_info in img_infos \
