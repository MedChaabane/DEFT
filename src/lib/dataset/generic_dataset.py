...
from utils.image import flip, color_aug, show_matching_hanlded_rectangle, ResizeShuffleBoxes, FormatBoxes, ToTensor, ToPercentCoordinates
...
class GenericDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if opt.AFE:
      ...
      pre_image_AFE,pre_image_AFE_org = self._get_input(pre_image_AFE, trans_input_pre_AFE)
      ...
      labels = np.repeat(np.expand_dims(np.array(track_ids_pre), axis=1), len(track_ids_next), axis=1) == np.repeat(np.expand_dims(np.array(track_ids_next), axis=0), len(track_ids_pre), axis=0)
      labels = np.pad(
            labels,
            [(0, max(0, self.max_object - labels.shape[0])), (0, max(0, self.max_object - labels.shape[1]))],
            mode="constant",
            constant_values=0,
        )
...
#       cv2.imwrite( '/s/red/a/nobackup/vision/chaabane/tracker/DEFT/src/matching.png', im)
      ...
      bboxes_pre, bboxes_next = ToPercentCoordinates(bboxes_pre, bboxes_next,inp_org)  
      bboxes_pre, bboxes_next, labels = ResizeShuffleBoxes(
            max_object=self.max_object, boxes_pre=bboxes_pre, boxes_next=bboxes_next, labels=labels
        )
  ...
  def _load_pre_data_AFE(self, video_id, frame_id, sensor_id=1):
    ...
    if 'train' in self.split:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          ...
          if abs(img_info['frame_id'] - frame_id) <= self.opt.max_frame_dist_AFE and abs(img_info['frame_id'] - frame_id)>0 and \
  ...
  def _load_pre_data(self, video_id, frame_id, sensor_id=1):
    ...
    if 'train' in self.split:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          for img_info in img_infos \
          if abs(img_info['frame_id'] - frame_id) < self.opt.max_frame_dist and img_info['frame_id'] - frame_id<0 and \
          (not ('sensor_id' in img_info) or img_info['sensor_id'] == sensor_id)]
    else:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          for img_info in img_infos \
