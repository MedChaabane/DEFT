...
class MOT(GenericDataset):
  ...
  def __init__(self, opt, split):
    ...
    if opt.dataset_version in ['17trainval', '17test','15trainval', '15test','15train']:
    ...
    elif opt.dataset_version == '15halftrain':
      ann_file = '{}.json'.format('train_half')
    elif opt.dataset_version == '15halfval':
      ann_file = '{}.json'.format('val_half')
...
class MOT_prediction(TrajectoryDataset):
  num_categories = 1
  default_resolution = [544, 960]
  class_name = ['']
  max_objs = 256
  ...
  def __init__(self, opt, split):
    ...
    self.images = None
    # load image list and coco
    super(MOT_prediction, self).__init__(opt, split, ann_path, img_dir)
